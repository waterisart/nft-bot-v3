	...
	socket.onmessage = async (msg) => {
		...
		for(const openTrade of knownOpenTrades.values()) {
			...
			if(orderType > -1) {
				...
				} catch(e					console.log("An unexpected error occurred trying to trigger an order (order type: " + orderTypes[triggeredOrderTrackingInfo.type] + ", nft id: " + availableNft.id + ")", error);
